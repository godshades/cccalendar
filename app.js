function ccCalendar() {
  ...
  return {
    ...
    get filteredCards() {
      ...
      return this.prebuiltCards.filter(function(name) {
    ...
    addSelectedCard: function() {
      if (!this.selectedCardName) {
        this.showToast('Vui lòng chọn thẻ', 'error');
        return;
      }
      if (!this.selectedCardDate || this.selectedCardDate < 1 || this.selectedCardDate > 28) {
        this.showToast('Vui lòng nhập ngày thanh toán (1-28)', 'error');
        return;
      }
      if (this.isCardAdded(this.selectedCardName)) {
        this.showToast('Thẻ này đã được thêm', 'error');
        return;
      }
      var randomChars = Math.random().toString(36).substring(2, 6);
      var uniqueId = 'card-' + Date.now() + '-' + randomChars;
      var self = this;
      var cardData = this.allCardsData.find(function(c) { return c.name === self.selectedCardName; });
      ...
      this.showToast('Đã thêm thẻ ' + this.selectedCardName, 'success');
